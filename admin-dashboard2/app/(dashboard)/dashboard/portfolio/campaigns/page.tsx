...
export default function PortfolioCampaignsPage() {
  ...
  const rows = data?.portfolio.topCampaignsMonth ?? [];
  ...
  return (
    <div className="space-y-6">
      ...
      <header className="flex flex-col gap-1 border-b border-purple-900/40 pb-4">
        <h1 className="text-2xl font-semibold tracking-tight">Portfolio · Campaigns</h1>
        <p className="text-sm text-muted-foreground">
          TZ: {data?.timezone ?? "Asia/Kathmandu"} • Uses <code>utm_campaign</code>
        </p>
      </header>

      <section className="rounded-2xl border border-purple-900/40 bg-linear-to-b from-[#0b0515] to-[#050316] p-6">
        <div className="mb-4">
          <h2 className="text-lg font-semibold">Top campaigns (this month)</h2>
          <p className="text-xs text-muted-foreground">
            Add UTMs like <code>?utm_source=instagram&utm_medium=social&utm_campaign=may2026&utm_content=reel1</code>
          </p>
        </div>
